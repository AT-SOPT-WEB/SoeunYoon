...
export default function Signup() {
  ...
  const isPwTooLong = password.length > 20 || confirmPw.length > 20;
  const isPwMismatch = password && confirmPw && password !== confirmPw;
      ...
      <div className="flex flex-col gap-6">
        {step === 1 && (
          <>
            ...
            />
            {isIdTooLong && (
              <p className="text-xs text-red-500">아이디는 20자 이하로 입력해주세요.</p>
            )}
          </>
        )}
          ...
          <>
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 top-8 text-zinc-600 hover:text-zinc-800"
              >
            ...
            {isPwTooLong && (
              <p className="text-xs text-red-500">비밀번호는 20자 이하로 입력해주세요.</p>
            )}
            {isPwMismatch && (
            ...
            <Button
              ...
              disabled={
                !!(
                  !canGoNextStep ||
                  (step === 1 && isIdTooLong) ||
                  (step === 2 && (isPwTooLong || isPwMismatch))
                )
              }
            >
              다음
